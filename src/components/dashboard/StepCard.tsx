    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          <span
            className={cn(
              "font-pixel text-[14px] w-8 shrink-0",
          ...
          <h3 className={cn(
            "font-pixel text-[14px] leading-loose",
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          {!isLocked && (
            <span className="font-mono text-sm text-[#6b6b8a]">{expanded ? "▲" : "▼"}</span>
          )}
        </div>
      </button>
      ...
      {expanded && !isLocked && (
        <div className="px-4 pb-4 flex flex-col gap-5 border-t border-[#1e1e2e]">
          <p className="font-mono text-sm text-[#a0a0b8] pt-4">{step.description}</p>

          {/* Tasks */}
          <div className="flex flex-col gap-2">
            <span className="font-pixel text-[11px] text-[#6b6b8a]">CHECKLIST</span>
            {step.tasks.map((task) => {
              ...
              return (
                ...
                >
                  <span className={cn(
                    "font-pixel text-[14px] mt-0.5 transition-colors shrink-0",
          ...
          {step.affiliates.length > 0 && (
            <div className="flex flex-col gap-2">
              <span className="font-pixel text-[11px] text-[#6b6b8a]">TOOLS FOR THIS STEP</span>
              <div className="flex flex-wrap gap-2">
                {step.affiliates.map((aff) => (
                  ...
                  >
                    <span className="font-mono text-sm text-[#a0a0b8] group-hover:text-[#e8e8f0]">
                      {aff.label}
                    </span>
                    {aff.note && (
                      <span className="font-pixel text-[11px] text-[#6b6b8a]">{aff.note}</span>
                    )}
                  </a>
                ))}
              </div>
          ...
          <div className="flex flex-col gap-3 border-t border-[#1e1e2e] pt-4">
            <div className="flex items-center justify-between">
              <span className="font-pixel text-[11px] text-[#06b6d4]">✨ AI OUTPUT</span>
          ...
          {!isCompleted && allTasksDone && (
            <button
              ...
              className="w-full font-pixel text-[14px] px-4 py-4 bg-[#10b981] text-[#0a0a0f] hover:bg-[#059669] transition-colors pixel-border"
